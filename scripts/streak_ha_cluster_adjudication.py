"""Close-streak + Heikin-Ashi cluster — DSR/SPA + OOS walk-forward adjudication.

Pre-registered in state/streak_ha_cluster_preregistration.json BEFORE this script
was run (2026-08-03). Decisive test of the two most suggestive specialized-setup
families added after the iter33 EUR/GBP NEGATIVE:
  - iter40 consecutive-close-streak continuation (run-length, 30th entry model)
  - iter42 Heikin-Ashi smoothed-candle strong-trend continuation (price-transform, 32nd)
Pools ALL fires of the 4 cluster setup types across all 14 symbols net-of-cost
(does NOT cherry-pick naive CI+ cells — that would compound selection bias).

Reuses existing DSR/SPA/fold math:
...
# ---- pre-registered cluster (must match state/streak_ha_cluster_preregistration.json) ----
CLUSTER_SYMBOLS = [
    "AUDUSDm", "BTCUSDm", "EURUSDm", "FR40m", "GBPUSDm", "JP225m",
    "NAS100m", "UK100m", "US30m", "US500m", "USDCHFm", "USDJPYm", "USOILm", "XAUUSDm",
]
CLUSTER_SETUPS = {
...
K_PRIMARY = 994         # honest full-search deflation (distinct_cells searched to date)
K_SECONDARY = 56       # cluster size (4 setup types x 14 symbols) — transparency only
K_TERTIARY = 2          # distinct entry models (close_streak, ha) — most-generous upper bound
...
def _dsr_suite(rs_net, sr_var):
    """DSR at the three pre-registered K levels."""
    out = {}
    for label, K in (("K994_primary", K_PRIMARY), ("K56_secondary", K_SECONDARY), ("K2_tertiary", K_TERTIARY)):
...
def main():
    cfg = load_config()
    print("=" * 78)
    print("Close-streak + Heikin-Ashi cluster — DSR/SPA + OOS adjudication")
    print("Pre-registered 2026-08-03 (state/streak_ha_cluster_preregistration.json)")
    print("=" * 78)

    # 1. Re-run replay_symbol for all 14 cluster symbols, collect per-fire rows.
    all_rows = []
    common_starts, common_ends = [], []
    for sym in CLUSTER_SYMBOLS:
        rows = replay_symbol(sym, cfg, max_bars=MAX_BARS)
    ...
    for sv_label, sv in (("sr_var_cluster", sr_var_cluster), ("sr_var_fixed_0.35", SR_VAR_FIXED)):
        ...
        print(f"  DSR [{sv_label}]: K994={suite['K994_primary']}  K56={suite['K56_secondary']}  K2={suite['K2_tertiary']}")
    ...
    print("\n--- PER-CELL DSR (K=994 primary, sr_var_cluster) top 12 by exp_net ---")
    print(f"  {'cell':28} {'n':>5} {'expN':>7} {'wr%':>5} {'sr':>5} {'DSR994':>7} {'DSR56':>7} {'DSR2':>6}")
    for c in eligible[:12]:
        rs = cells[(c["symbol"], c["setup"])]
        wr = round(100 * sum(1 for r in rs if r > 0) / len(rs), 1)
        s994 = round(deflated_sharpe(rs, n_trials=K_PRIMARY, sr_var_across_trials=sr_var_cluster), 4)
        s56 = round(deflated_sharpe(rs, n_trials=K_SECONDARY, sr_var_across_trials=sr_var_cluster), 4)
        s2 = round(deflated_sharpe(rs, n_trials=K_TERTIARY, sr_var_across_trials=sr_var_cluster), 4)
        print(f"  {c['cell']:28} {c['n']:>5} {c['exp_net']:>+7.3f} {wr:>5.1f} {c['sharpe_net']:>5.2f} {s994:>7.3f} {s56:>7.3f} {s2:>6.3f}")
    ...
    for sp in splits:
        ...
        d994 = round(deflated_sharpe(fr_net, n_trials=K_PRIMARY, sr_var_across_trials=sr_var_cluster), 4)
        ...
        fold_results.append({
            ...
            "ci95_lo": round(lo, 4), "ci95_hi": round(hi, 4), "DSR994": d994,
        })
        print(f"  fold {sp['fold']}: test {ts0}..{ts1}  n={len(fr_net):>4}  exp_net={expn:+.3f}  wr={wr:.1f}%  "
              f"CI95=[{round(lo,4)},{round(hi,4)}]  DSR994={d994}")
    folds_pos = sum(1 for f in fold_results if f.get("exp_net", 0) > 0)
    folds_dsr = sum(1 for f in fold_results if f.get("DSR994", 0) >= 0.95)
    folds_ci = sum(1 for f in fold_results if f.get("ci95_lo", -1) > 0)
    print(f"  fold consistency: {folds_pos}/{FOLDS} positive exp, {folds_ci}/{FOLDS} CI lo>0, {folds_dsr}/{FOLDS} DSR994>=0.95")
    ...
    print("=" * 78)
    pooled_dsr994 = deflated_sharpe(pooled_net, n_trials=K_PRIMARY, sr_var_across_trials=sr_var_cluster)
    spa_p = spa.get("p_consistent", spa.get("p", 1.0)) if spa else 1.0
    pooled_ci_lo = bootstrap_expectancy_ci(pooled_net, n_boot=2000)[0]
    checks = {
        "DSR994>=0.95 (pooled)": pooled_dsr994 >= 0.95,
        "SPA p<0.05": spa_p < 0.05,
        ">=2/3 folds DSR994>=0.95": folds_dsr >= 2,
    ...
    print(f"  (primary honest bar K=994; see pre-registration for K-secondary/tertiary transparency)")
    ...
    out = {
        "pooled": {"n": len(pooled_net), "exp_net": round(sum(pooled_net)/len(pooled_net),4),
                   ...
                   "DSR994": round(pooled_dsr994,4),
                   "DSR56": round(deflated_sharpe(pooled_net,n_trials=K_SECONDARY,sr_var_across_trials=sr_var_cluster),4),
                   "DSR2": round(deflated_sharpe(pooled_net,n_trials=K_TERTIARY,sr_var_across_trials=sr_var_cluster),4),
        ...
        "top_cells": cell_stats[:12],
    }
    outp = ROOT / "state" / "streak_ha_cluster_adjudication.json"
