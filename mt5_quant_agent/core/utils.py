...
# Context-aware PROJECT_ROOT resolution (2026-07-26)
# The bot's data_loop / fast_tick_loop / execution_loop write state files like
# paper_orders.json directly to project-root state/, but modules inside
# mt5_quant_agent/ import core.utils with __file__ at mt5_quant_agent/core/
# utils.py so Path(__file__).resolve().parent.parent yields mt5_quant_agent/
# and STATE_DIR ends up at mt5_quant_agent/state/ for those modules. That split
# caused the canonical account.json writer in mt5_connection_manager._try_path
# to land in mt5_quant_agent/state/account.json instead of the project-root
# state/account.json the dashboard reads. Detection: if start.py (the orchestrator)
# is one level up, the package is sub-mounted inside a real project root, and
# STATE_DIR should resolve to the uppermost state/ directory.
_base = Path(__file__).resolve().parent.parent
if (_base.parent / "start.py").exists():
    PROJECT_ROOT = _base.parent
else:
    PROJECT_ROOT = _base
